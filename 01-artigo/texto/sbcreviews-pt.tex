...
A solução foi estruturada como uma aplicação web em arquitetura cliente-servidor, organizada em camadas. Essa definição atende ao escopo do projeto, que envolve uma área pública voltada à apresentação institucional e comercial da empresa e uma área administrativa restrita para gestão de conteúdo.

No frontend, foi definido o uso de Next.js com TypeScript. Nesta proposta, o Next.js será utilizado como aplicação frontend da solução, responsável pela interface pública do portal e pelas rotas administrativas protegidas, consumindo uma API própria desenvolvida separadamente no backend. O uso de TypeScript contribui para maior previsibilidade no código e melhora a manutenção da aplicação ao longo do desenvolvimento.

Para a camada de interface, foi definido o uso de Tailwind CSS e de componentes personalizados construídos a partir de shadcn/ui. O shadcn/ui não será empregado como conjunto visual pronto, mas como base técnica para a estruturação de componentes próprios, coerentes com o sistema visual do portal. Essa definição responde tanto à necessidade de componentização quanto à necessidade de evitar uma interface genérica.

No backend, foi definido o uso de NestJS com TypeScript. A escolha foi feita pela compatibilidade com uma organização modular adequada a um sistema que precisa gerenciar conteúdos institucionais, produtos, eventos, itens de FAQ e mensagens de contato.

Para a persistência, foi definido o uso de PostgreSQL como banco de dados relacional e Prisma ORM como ferramenta de acesso aos dados. Essa combinação é compatível com a estrutura prevista para o sistema, que inclui entidades bem definidas, como usuários administrativos, notícias, eventos, produtos, itens de FAQ e mensagens de contato.

A autenticação da área administrativa será baseada em JWT, restringindo o acesso ao painel de gestão de conteúdo. Também foi definida a adoção de Docker como estratégia de containerização prevista para o ambiente de desenvolvimento e para a futura implantação. Em conformidade com as diretrizes da linha Web Apps, a proposta não depende de plataformas no-code, bancos locais ou serviços que substituam backend próprio.

Com base nessas definições, a stack final do projeto ficou organizada da seguinte forma: Next.js, TypeScript, Tailwind CSS e componentes personalizados baseados em shadcn/ui no frontend; NestJS, TypeScript e Prisma no backend; PostgreSQL na persistência de dados; JWT na autenticação administrativa; e Docker como estratégia de containerização.
